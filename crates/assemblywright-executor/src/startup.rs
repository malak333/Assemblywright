...
fn validate_service_name(name: &str) -> Result<(), RuntimeError> {
    let fixture = name.strip_prefix("AssemblywrightExecutorE2E");
    if name == "AssemblywrightExecutor"
...
mod tests {
    ...
    fn service_name_and_config_boundaries_are_exact() {
        let digest = "00".repeat(32);
        let path = if cfg!(windows) { "C:\\a" } else { "/a" };
        let maximum = format!("AssemblywrightExecutorE2E{}", "a".repeat(32));
        assert!(matches!(
            parse_startup_args(args(&[
                "--service-host",
                "--service-name",
        ...
        for name in [
            ...
            "AssemblywrightExecutorE2Ebad-name",
            &format!("AssemblywrightExecutorE2E{}", "a".repeat(33)),
        ] {
            assert_eq!(
                parse_startup_args(args(&[
                    "--service-host",
