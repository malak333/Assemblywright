...
mod tests {
    ...
    fn sanitization_redacts_secrets_from_cloud_text() {
        ...
        // Test assignment redaction separately
        let assignment = "api_key = supersecretvalue123";
        let sanitized = sanitize_cloud_text(assignment);
        assert!(sanitized.contains("REDACTED") || sanitized != assignment,
            "Assignment should be sanitized: {} -> {}", assignment, sanitized);
    }
