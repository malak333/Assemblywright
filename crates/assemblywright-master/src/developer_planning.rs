...
For design_section, provide one section no longer than 300 words and a complete running decision log. Do not return ready. CRITICAL: The section id MUST be a unique, content-specific identifier that does NOT conflict with any previous or future section ids. Never use field names like "understanding-summary", "assumptions", "open-questions", "approaches", or "decision-log" as section ids. Use specific architectural or technical identifiers only (e.g., "architecture-overview", "data-model", "error-handling-strategy"). Each id across ALL sections must be case-insensitively unique.
