...
clear_live_environment() {
  ...
  while IFS='=' read -r name _value; do
    case "$name" in
      GIT_* | BASH_ENV | ENV | ASSEMBLYWRIGHT_MAC_BRIDGE_BIN | ASSEMBLYWRIGHT_MAC_AGENT_BIN | ASSEMBLYWRIGHT_TAILSCALE_BIN | ASSEMBLYWRIGHT_RESTRICTED_WORKER_INTERNAL_STDIN_V1 | ASSEMBLYWRIGHT_RESTRICTED_WORKER_INTERNAL_ROOT | ASSEMBLYWRIGHT_RESTRICTED_WORKER_RECEIPT_FD)
        unset "$name"
        ;;
    esac
  done < <(env)
...
EXPECT
  ...
  initialize_fixture "$environment_hardening" \
    "$coordination_body"$'\n'"[[ -z \"\${GIT_DIR:-}\${GIT_WORK_TREE:-}\${GIT_INDEX_FILE:-}\${GIT_OBJECT_DIRECTORY:-}\${GIT_ALTERNATE_OBJECT_DIRECTORIES:-}\${GIT_REPLACE_REF_BASE:-}\${ASSEMBLYWRIGHT_MAC_BRIDGE_BIN:-}\${ASSEMBLYWRIGHT_MAC_AGENT_BIN:-}\${ASSEMBLYWRIGHT_TAILSCALE_BIN:-}\" ]]"$'\n'"$terminal_body"
