...
write_signed_distribution_provenance() {
  ...
  require_output_contains "app bundle Gatekeeper assessment" "$app_gatekeeper" "accepted"
  require_output_contains "installer package Gatekeeper assessment" "$pkg_gatekeeper" "accepted"
...
assert "accepted" in data["gatekeeper"]["app_bundle_assessment"]
assert "accepted" in data["gatekeeper"]["installer_pkg_assessment"]
