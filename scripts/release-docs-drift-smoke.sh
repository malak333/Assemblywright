...
require_file "$FEATURE_CONVEYOR_DESIGN"
...
require_text "knowledge base phase unit-test rule" "$KB" "analyze changed testable units"
require_text "knowledge base phase E2E applicability rule" "$KB" "browser/UI automation is not applicable"
...
require_text "feature conveyor approved design" "$FEATURE_CONVEYOR_DESIGN" "Status: APPROVED by owner"
require_text "feature conveyor default-inert kernel boundary" "$FEATURE_CONVEYOR_DESIGN" "default-inert"
require_text "feature conveyor Windows master ownership" "$FEATURE_CONVEYOR_DESIGN" 'Windows `jarvis-master`'
require_text "master Feature Conveyor schema" "$MASTER_CRATE" "pub const MASTER_SCHEMA_VERSION: i64 = 5;"
require_text "master Feature Conveyor capacity" "$MASTER_CRATE" "MAX_CONVEYOR_NONTERMINAL_FEATURES"
require_text "master Feature Conveyor startup quarantine" "$MASTER_CRATE" "feature_startup_quarantines"
require_text "architecture Feature Conveyor kernel" "$ARCHITECTURE" "schema-v5 Feature Conveyor repository kernel"
require_text "build docs Feature Conveyor unit proof" "$BUILD_DOCS" "cargo test -p jarvis-master --lib feature_conveyor_unit --locked -- --nocapture"
require_text "build docs Feature Conveyor E2E proof" "$BUILD_DOCS" "cargo test -p jarvis-master --test feature_conveyor_kernel --locked -- --nocapture"
require_text "build docs Feature Conveyor Playwright boundary" "$BUILD_DOCS" "Playwright/browser E2E is not applicable"
require_text "release checklist Feature Conveyor boundary" "$CHECKLIST" "Durable Feature Conveyor schema-v5"
require_text "knowledge base Feature Conveyor boundary" "$KB" "schema-v5 repository kernel"
require_text "README Feature Conveyor boundary" "$README" "Durable Feature Conveyor"
require_text "root design Feature Conveyor boundary" "$DESIGN" "schema-v5 repository"
require_text "safety rules Feature Conveyor boundary" "$SAFETY_RULES" "schema-v5 Durable Feature Conveyor kernel"
...
require_text "master schema version" "$MASTER_CRATE" "pub const MASTER_SCHEMA_VERSION: i64 = 5;"
...
require_text "master enrollment schema migration E2E" "$MASTER_IDENTITY_E2E" "schema_v1_migrates_transactionally_through_feature_conveyor_v5"
