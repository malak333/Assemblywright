...
COMMAND_OUTPUT="$("$BUNDLED_CORE" command "plugin echo packaged-layout smoke" --endpoint "$ENDPOINT")"
...
BLOCKED_OUTPUT="$("$BUNDLED_CORE" command "plugin echo blocked by packaged-layout pause" --endpoint "$ENDPOINT" --dry-run)"
