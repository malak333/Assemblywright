...
require_text "ASSEMBLYWRIGHT_RELEASE_LOCAL_HEARTBEAT_SECONDS: \"60\"" "$WORKFLOW"
...
require_text "cargo clippy -p assemblywright-protocol --all-targets --locked -- -D warnings" "$WINDOWS_PROTOCOL_WORKFLOW"
require_text "cargo clippy -p assemblywright-master --all-targets --locked -- -D warnings" "$WINDOWS_PROTOCOL_WORKFLOW"
require_text "cargo clippy -p assemblywright-broker -p assemblywright-executor --all-targets --locked -- -D warnings" "$WINDOWS_PROTOCOL_WORKFLOW"
require_text "cargo test -p assemblywright-protocol --locked" "$WINDOWS_PROTOCOL_WORKFLOW"
require_text "cargo test -p assemblywright-master --locked" "$WINDOWS_PROTOCOL_WORKFLOW"
require_text "cargo test -p assemblywright-broker --locked -- --nocapture" "$WINDOWS_PROTOCOL_WORKFLOW"
require_text "cargo test -p assemblywright-executor --locked -- --nocapture" "$WINDOWS_PROTOCOL_WORKFLOW"
require_text "cargo test -p assemblywright-master --test windows_repository_onboarding_e2e --locked -- --nocapture" "$WINDOWS_PROTOCOL_WORKFLOW"
require_text 'ASSEMBLYWRIGHT_REQUIRE_WINDOWS_SERVICE_E2E: "1"' "$WINDOWS_PROTOCOL_WORKFLOW"
require_text "cargo test -p assemblywright-master --test windows_service_lifecycle_e2e --locked -- --ignored --nocapture" "$WINDOWS_PROTOCOL_WORKFLOW"
