...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='assemblywright-model-target-e2e-') as temp:
        ...
        try:
            ...
            assert 'queue_v11' in durable and 'queue_v4' not in durable and 'queue_v3' not in durable
            assert next(feature for feature in durable['queue_v11'] if feature['id'] == persisted_id)['model_target'] == 'windows'
            ...
            print(json.dumps({
                ...
                'queue_v11_restart_preserves_target': True,
            }))
        finally:
