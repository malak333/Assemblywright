...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="assemblywright-developer-review-e2e-") as temp:
        ...
        try:
            ...
            assert "queue_v11" in durable
            durable_repaired = next(f for f in durable["queue_v11"] if f["id"] == repaired_id)
            ...
            durable_restarted = next(f for f in durable["queue_v11"] if f["id"] == restart_id)
            ...
            print(json.dumps({
                ...
                "durable_queue_schema": "queue_v11",
            }))
        finally:
            try:
                call("emergency")
