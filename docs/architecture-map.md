...
flowchart TB
    ...
    subgraph Core["jarvis-core"]
        ...
        IPC --> Inspection["/tasks, /audit, /activity/summary, /model-routes, /memory, /permissions/grants, /permissions/policy-review, /plugins/manifests, /plugins/installed"]
...
status counts, active task count, recent tasks, and recent audit entries. This
is pollable progress evidence for CLI and Swift; it is not a long-lived
streaming channel.
...
| IPC boundary | Axum loopback HTTP JSON API for health, commands, task/audit/activity-summary/model-route/memory/approval inspection, plugin manifests, emergency pause, and scheduler jobs. | Versioned, compatibility-tested app/core API with packaged app smoke coverage and clear degraded-mode handling. | Core IPC implemented; production app contract hardening pending. |
| Command runtime | `ConversationRuntime` creates tasks, runs a routed `ModelExecutor` (`FakeLocalModel` by default, Ollama-compatible HTTP or ChatGPT/OpenAI-compatible HTTP when explicitly enabled), records structured audit entries, handles pause/cancel, enforces max steps, can persist task/audit/model-route state through `RuntimeCommandStore`, exposes repository-backed activity summaries for pollable progress visibility, and can execute bounded model-planned first-party tool calls after schema and policy checks. | Multi-step assistant runtime with production model responses, installed plugin/tool orchestration, streaming progress, approval UI handoff, and robust recovery. | Bounded fake-model tool orchestration, opt-in local and ChatGPT provider boundaries, explicit installed-plugin subprocess runner, CLI/IPC/Swift approval scaffold, and pollable activity summaries implemented; long-lived streaming pending. |
