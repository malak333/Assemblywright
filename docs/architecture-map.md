...
dispatching plugin code. It supports opt-in ChatGPT/OpenAI-compatible execution
only after route policy allows it. It does not yet support installed plugin
sandboxing or a signed packaged Mac approval flow.
...
| Scheduler | Inspectable scheduler jobs with manual, one-time, interval trigger contracts, run-due execution, visible task/audit records, deterministic due ordering, and cancellation support. Repository-backed IPC state restores and updates jobs through SQLite. Emergency pause cancels active scheduler jobs, and unsafe due commands fail closed by pausing and cancelling remaining open jobs. It does not yet run a background clock. | Durable scheduler and trigger engine for approved proactive routines, persisted job state, visible task records, and policy-gated execution. | Durable job state and explicit run-due execution implemented; background trigger loop pending. |
