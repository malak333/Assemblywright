...
`27428860335` / job `81073692261`. After PR #288, `main` at `69c9fef`
...
`pending_manual_validation`, while scheduler notification UI evidence has
delivered-notification field presentation and reset coverage in the app layer,
voice capture is gated behind permission state before start attempts, and the
Release tab surfaces runbook-load warnings without converting local evidence
inspection into production proof.
`jarvis release evidence-status --json` remains expected to report
`complete: false` with six missing external/manual evidence artifacts after
the local generated app presence artifacts exist. Fresh
