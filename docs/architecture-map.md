...
flowchart TB
    ...
    subgraph Core["jarvis-core"]
        ...
        IPC --> SchedulerRecovery["/scheduler/recover-stale explicit recovery"]
...
- `jarvis-core::scheduler`: Inspectable scheduler jobs with manual, one-time,
  ...
  support. Jobs are in-memory when the IPC state has no repository and
  restored/updated through SQLite when repository backing is enabled.
