...
flowchart TB
    ...
    MacShell --> MemoryUI["Memory CRUD, classification, review, soft-delete, restore, and count-only index governance"]
...
flowchart TB
    ...
    subgraph CoreProd["jarvis-core production runtime"]
        ...
        FirstPartyPlugins --> WorkspaceRootsProd["app-owned explicit workspace root grants"]
    ...
    VectorGovernance --> SemanticRetrieval["future semantic retrieval and context injection"]
...
| Plugins and tools | Production inventory excludes deterministic `fake_*` fixtures and uses one configured `PluginHost` for IPC inspection, provider advertisement, direct dispatch, approval replay, and runtime execution. `system_status.status` is always present; explicit repeatable root configuration adds local-only `workspace_inspect.list` and `workspace_inspect.read_text`. Workspace traversal is descriptor-anchored and no-follow, rejects traversal, symlink, hidden/credential-like, special, binary, and oversized targets, returns bounded untrusted data, records metadata-only audit, and lets pause/cancel/timeout dominate late completion. `/tools/model` and the Swift Plugin tab expose redacted risk/scope/constraint metadata without roots, paths, schemas, or results. Local plugin installation still validates provenance and explicit subprocess grants with bounded JSON I/O, while truthfully recording that OS sandbox and egress enforcement are not yet present. | App-owned workspace-root grants and bookmarks, OS sandboxed first-party and installed tools, content classification, durable provider-originated approval replay for sensitive reads, user grant UI, proactive gating, progress handoff, and production model-generated tool execution. | Bounded production status and explicitly configured workspace read-only tools, local-model-only result containment, runtime-derived inventory, cooperative cancellation, metadata-only audit, CLI/IPC/Swift inspection, unit and cross-process E2E implemented; app-owned root selection/bookmarks, broader WASM isolation, OS sandbox/egress enforcement, provider-originated sensitive-read approval replay, marketplace trust, and live-device QA pending. |
...
| Storage and memory | SQLite migrations store tasks, audit/model-route evidence, emergency pause, canonical memory items, scheduler jobs, approvals, and plugin metadata. A versioned sibling memory-index manifest is atomically rebuilt from active canonical rows; status detects missing, stale, deleted, orphaned, corrupt, and current projections while returning counts only. CLI/IPC and Swift expose explicit status/rebuild controls. | SQLite remains canonical; the manifest is local-only, rebuildable, and never a model/cloud input. Keychain owns secrets. | CRUD/review/retention plus rebuildable index governance, restart persistence, corruption recovery, redacted Swift status, Rust tests, and cross-process CLI E2E implemented; semantic/vector retrieval, context injection, and autonomous rewrite/purge remain pending. |
...
- The local index manifest is rebuildable from canonical active records and
  never becomes source of truth; semantic retrieval remains future work.
...
explicit rebuild, and model refresh tests. Cross-process CLI coverage proves
missing/corrupt recovery, mutation staleness, restart persistence, delete cleanup,
and restore invalidation. It does not prove semantic retrieval, model-context
injection, purge automation, or memory rewriting.
