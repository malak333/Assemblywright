...
## Atomic approved-execution slice

Current implementation:

```mermaid
flowchart LR
  A["Approved pending_approvals record"] --> V["Validate task, exact action, scopes, current manifest, input schema, and policy"]
  V --> C["BEGIN IMMEDIATE: insert unique schema-v13 approval_executions claim plus redacted claim audit"]
  C --> P["Release repository mutex; enter configured first-party plugin"]
...
flowchart LR
  ...
  G --> O["Durable single-consumer effect coordinator"]
...
The current claim prevents concurrent or restart replay from consuming the
same approval twice, but it does not prove that an external system applied a
...
flowchart TB
    ...
    LocalGate --> MigrationSmoke["storage-migration-backup-smoke.sh backup, restore, newer-schema, and v1-v12 fixture proof"]
    ...
    subgraph Core["jarvis-core"]
        ...
        ApprovalExecute --> ApprovalPreclaim["task, action, scope, manifest, input schema, and current-policy validation"]
    ...
    SchemaV13 --> HistoricalFixtureMatrix["representative legacy fixture preservation"]
...
representative schema v1-v12 fixture preservation for persisted repository
