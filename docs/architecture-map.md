...
| Plugins and tools | Production inventory excludes deterministic `fake_*` fixtures and uses one configured `PluginHost` for IPC inspection, provider advertisement, direct dispatch, approval replay, and runtime execution. `system_status.status` is always present; explicit repeatable root configuration adds local-only bounded workspace inspection. Installed `local_wasm` plugins require `wasm_compute`, exact-byte provenance, the no-import `jarvis_json_v1` exports, low-risk compute-only policy, and hard module/request/output/memory/fuel limits; pause/cancel/timeout/fuel exhaustion fail closed. Redacted inspection and Swift presentation distinguish Wasmi confinement from `local_subprocess`, which remains not OS sandboxed. | App-owned workspace-root grants and bookmarks, defense-in-depth OS sandboxing for first-party, WASM, and subprocess tools, content classification, durable approval replay, user grant UI, publisher/marketplace/malware trust, and production model-generated tool execution. | Bounded workspace tools plus no-import Wasmi language confinement, schema-v12 migration, exact-byte provenance, redacted IPC/Swift inspection, unit and cross-process E2E implemented; app-owned root selection/bookmarks, OS sandbox/egress enforcement, same-user IPC, marketplace/publisher/malware trust, signing/notarization, and live-device QA pending. |
