...
kept in `docs/distributed-developer-mode-design.md`.
...
installed record or because Assemblywright can parse and snapshot it. A successful
...
flowchart LR
  ...
  F --> L["Explicit assemblywright --ipc-token-file client"]
  L --> M
...
```mermaid
flowchart LR
  A["Developer ID signed and notarized Assemblywright.app plus stable core identity"] --> B["Owner-recorded signed clean-profile validation"]
...
Assemblywright is a local-first macOS assistant foundation. The current repository
...
flowchart TB
    ...
    LocalGate --> SignedDistributionRunbook["assemblywright release signed-distribution-runbook read-only signed artifact triage"]
    LocalGate --> LiveDeviceQA["release-live-device-qa.sh check/self-test preflight"]
    LocalGate --> PluginTrustRunbook["assemblywright release plugin-trust-runbook read-only plugin trust triage"]
    ...
    ReleaseReadiness["/release/readiness and assemblywright release readiness"] --> Docs
    ReleaseReadinessFallback["serverless CLI readiness fallback"] --> ReleaseReadiness
    EvidenceStatus["/release/evidence-status and assemblywright release evidence-status"] --> EvidenceDoctor
    ...
    UnsignedStructure["package-distribution.sh --unsigned-structure-check"] --> ReleaseApp["target/distribution/Assemblywright.app"]
    ...
    subgraph Core["jarvis-core"]
        ...
        InstalledAuthorityMutation --> RedactedLifecycleResponse
        ...
        InstalledProvenanceVerify --> ProvenanceCheck
        InstalledEnable --> InstalledAuthorityAudit
    ...
    OwnerLease --> OwnerLeaseBoundary["cooperating Assemblywright owners only; raw SQLite writers not OS-blocked"]
...
the selected `JARVIS_CHATGPT_REASONING_EFFORT`, and an explicit
`JARVIS_CHATGPT_REQUIRES_APPROVAL` policy. The Model tab offers Codex-account
model and compatible effort pickers. Its `Ask before every cloud prompt` toggle
can remove repeated approval for ordinary conversation while Private and
Credential-adjacent routes retain one-shot approval, Restricted routes remain
blocked, and proactive/tool-action gates are unchanged. `OpenAI API` uses the configured
...
When Rust returns `waiting_for_approval` for a sensitive Console cloud route or
an operator-selected every-prompt policy, Swift retains
...
Assemblywright passes redacted context over stdin, uses a private temporary response
...
rechecks before reading. The Assemblywright payload contains only redacted route
...
`Assemblywright.app` payload. The report is required by
...
owner-recorded external evidence is present; Assemblywright has not itself performed
signing, notarization/stapling, live-device QA, plugin trust QA, or manual
release QA.
`assemblywright release readiness` prefers that IPC endpoint and falls back to the same
...
`/release/evidence-status` and `assemblywright release evidence-status` expose the
...
`/approvals/:id/execute` or `assemblywright approvals execute <approval-id>`. The path
...
then verifies `assemblywright release evidence-status` accepts that report and
...
`assemblywright serve --scheduler-recover-stale-on-startup` can run the same recovery
...
Assemblywright verifies an Ed25519 signature over the portable unsigned manifest
...
its lifetime, serializing cooperating Assemblywright cores across backup, migration,
...
installed-plugin records through the redacted inspection contract and owns
typed verify, explicit compatible-grant enable, and disable controls. Local paths,
...
remain visible. It shows exact declared permissions and network hosts, requires
matching provenance and explicit selection for mixed grants, binds the exact
confirmed grant and lifecycle-contract digest as a compare-and-set, serializes
each plugin mutation, and refreshes the authoritative record after every
outcome. A stale registry disables all lifecycle actions, and verify/set
responses retain the redacted inspection projection. Rust transactionally
records provenance audit evidence and atomically records each authority change
with a redacted non-execution audit; Swift never installs or runs plugin code.
If the installed registry endpoint is unavailable, the tab
...
or failure audit entries plus bounded model-output chunk metadata. Assemblywright
...
`NSSpeechRecognitionUsageDescription` is `Assemblywright uses speech recognition only to turn your spoken command into a local assistant request.`. Owner-recorded evidence-note fields must contain
...
`assemblywright release evidence-status` accepts both the plugin-trust QA report and
...
- `jarvis-cli`: Local CLI for serving the IPC API with optional `--db-path`
  ...
  unavailable-IPC recovery guidance, and running `assemblywright smoke` against
...
```mermaid
flowchart TB
    MacApp["Developer ID signed, notarized, stapled, installed Assemblywright.app"]
    ...
    UX --> PluginManagerProd["installed plugin verify, explicit grant enable, disable, trust review, and logs"]
    ...
    subgraph CoreProd["jarvis-core production runtime"]
        ...
        OwnerLeaseProd --> CooperativeBoundaryProd["cooperating Assemblywright owners; raw writers remain outside advisory enforcement"]
        ...
        InstalledPlugins --> InstalledLifecycleProd["audited provenance and execution-authority lifecycle"]
        InstalledLifecycleProd --> RepoProd
...
Start until the selected model is installed. For a loopback endpoint, the same
tab exposes an explicitly confirmed Homebrew formula upgrade that uses a fixed
executable without a shell, filters inherited environment values, verifies the
version before and after upgrade, and restarts Ollama only when its Homebrew
service was already running. Remote endpoints, non-formula installs, failed
checks, and stopped services remain fail-closed or manual. A process-level Swift
E2E drives the model through the real Foundation process runner and a temporary
fake Homebrew executable, proving version and restart effects without mutating
the host installation. Swift focused integration tests
cover the model/controller path, and `JarvisMacAppTests` covers the app-level
Start/Download/Stop/Upgrade gating plus progress presentation without requiring a live
Ollama daemon.
...
`gh run list --branch main --workflow "Assemblywright Release Local Gate" --limit 3`.
The
...
`assemblywright release evidence-status --json` remains expected to report
...
unsigned distribution smoke evidence. It does not support a claim that Assemblywright is
