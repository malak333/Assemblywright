...
| IPC boundary | Axum loopback HTTP JSON API for health, commands, task/audit/activity-summary/activity-events/model-route/memory/approval inspection, plugin manifests, emergency pause, scheduler jobs, and `/contract` feature metadata that lists implemented surfaces with proof and production boundaries. | Versioned, compatibility-tested app/core API with packaged app smoke coverage, feature/boundary negotiation, and clear degraded-mode handling. | Core IPC plus feature/boundary contract metadata implemented; broader production compatibility policy pending. |
