...
| --- | --- | --- | --- |
| Mac shell | Buildable Swift/SwiftUI scaffold with health, command transcript, pause/resume, activity/audit rendering, bounded activity event-stream watch, memory classification and create/update/review/delete/restore controls, plugin/scheduler/diagnostics tabs, degraded-mode handling, and a `JarvisCoreSupervisor` abstraction for configured or bundled local core binaries. The Plugin tab renders first-party manifests plus installed-plugin registry records with source path, execution grant, provenance integrity, origin-review, and executable status while degrading gracefully when the repository-backed installed registry is unavailable. The local packaged app smoke assembles and ad-hoc signs a deterministic `Jarvis.app` for temp-profile launch proof. | Packaged `Jarvis.app` supervises the core, owns voice/text UX, settings, memory and permission surfaces, diagnostics export, and recovery states. | Shell supervision scaffold, Swift memory management, bounded activity event-stream watch, installed-plugin registry inspection surfaces, and local assembled-app smoke implemented; Developer ID signing, notarization, installer/restart proof, and production release QA pending. |
